...
namespace gazebo {
 ...
 // Load the controller
  void FourWheelDriveMultiWheel::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    if (!_sdf->HasElement("left_joint_front")) {
      gzthrow("Have to specify space separated left side joint names via <leftJoints> tag!");
    } else {
      std::string joint_string = _sdf->GetElement("left_joint_front")->Get<std::string>();
      boost::split( joint_names_[FRONT_LEFT], joint_string, boost::is_any_of(" ") );
    }

    //this->right_joint_names_ = "right_joint";
    if (!_sdf->HasElement("right_joint_front")) {
      gzthrow("Have to specify space separated right side joint names via <rightJoints> tag!");
    } else {
      std::string joint_string = _sdf->GetElement("right_joint_front")->Get<std::string>();
      boost::split( joint_names_[FRONT_RIGHT], joint_string, boost::is_any_of(" ") );
    }

    //this->left_joint_names_ = "left_joint";
    if (!_sdf->HasElement("left_joint_rear")) {
      gzthrow("Have to specify space separated left side joint names via <leftJoints> tag!");
    } else {
      std::string joint_string = _sdf->GetElement("left_joint_rear")->Get<std::string>();
      boost::split( joint_names_[REAR_LEFT], joint_string, boost::is_any_of(" ") );
    }

    //this->right_joint_names_ = "right_joint";
    if (!_sdf->HasElement("right_joint_rear")) {
      gzthrow("Have to specify space separated right side joint names via <rightJoints> tag!");
    } else {
      std::string joint_string = _sdf->GetElement("right_joint_rear")->Get<std::string>();
    ...
    if (!_sdf->HasElement("publishOdometryMsg")) {
      ROS_WARN("FourWheelDriveMultiWheel Plugin (ns = %s) missing <publishOdometryMsg>, defaults to %s", this->robot_namespace_.c_str(), this->publish_odometry_msg_ ? "true" : "false");
    } else {
      this->publish_odometry_msg_ = _sdf->GetElement("publishOdometryMsg")->Get<bool>();
      this->odometry_publisher_ = rosnode_.advertise<nav_msgs::Odometry>("odom",100);
}
   ...
   ROS_INFO("Starting FourWheelDriveMultiWheel Plugin (ns = %s)!", this->robot_namespace_.c_str());

    // ROS: Subscribe to the joint command topic 
     // this->joint_command_subscriber_ = this->rosnode_.subscribe<base_drive_chain::BaseState>(this->command_topic_, 100, boost::bind(&FourWheelDriveMultiWheel::jointcmdVelCallback, this, _1));
   ros::SubscribeOptions so = ros::SubscribeOptions::create<base_drive_chain::BaseState>(command_topic_, 100, boost::bind(&FourWheelDriveMultiWheel::jointcmdVelCallback, this, _1), ros::VoidPtr(), &this->queue_);
    joint_command_subscriber_ = this->rosnode_.subscribe(so);

   // start custom queue for diff drive
   this->callback_queue_thread_ = boost::thread(boost::bind(&FourWheelDriveMultiWheel::QueueThread, this));

   this->update_connection_ = event::Events::ConnectWorldUpdateBegin(boost::bind(&FourWheelDriveMultiWheel::UpdateChild, this));
}
